...
class PresenceSensorAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    super(
      ...
      homebridgeAccessory,
      deviceConfig,
      Accessory.Categories.SENSOR, //TODO: MotionSensor
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      ...
      this.log.log('statusMap.code: ', statusMap.code);
      if (statusMap.code === 'pir') {
        ...
        this.log.log('hbSensorState:', hbSensorState)
  ...
  tuyaParamToHomeBridge(name, param) {
    ...
    switch (name) {
      case Characteristic.ContactSensorState:
        this.log.log('Characteristic.ContactSensorState:',Characteristic.ContactSensorState);
        let status
        if (param === true) {
  ...
  normalAsync(name, hbValue) {
    this.log.log('normalAsync: name: %s \n, hbValue):%s', name,hbValue);
  ...
  getAccessoryCharacteristic(name) {
    ...
    this.log.log('AHR: getAccessoryCharacteristic(name):', name);
  ...
  updateState(data) {
    this.log.log('updateState(data):', data);
